...
class AdminAttendanceController extends Controller
{
    ...
    public function userIndex(Request $request, User $user){
        ...
        // ② 対象ユーザーの当月レコードを取得（← auth()->id() ではなく $user->id）
        $records = Attendance::query()
            ->with('breaks')
            ->where('user_id', $user->id)   // ← ここだけ違う
            ->whereBetween('date', [$start->toDateString(), $end->toDateString()])
            ->orderBy('date')
            ->get()
            ->keyBy(fn ($r) => $r->date->format('Y-m-d'));

        // ③ 既存レコードで上書き（一般ユーザーと同ロジック）
    ...
    public function update(AdminAttendanceUpdateRequest $request, Attendance $attendance)
    {
        ...
        // 出退勤
        $attendance->clock_in  = array_key_exists('clock_in',  $validated) ? $toHis($validated['clock_in'])  : $attendance->clock_in;
        $attendance->clock_out = array_key_exists('clock_out', $validated) ? $toHis($validated['clock_out']) : $attendance->clock_out;
        $attendance->save();
        ...
        foreach (($validated['breaks'] ?? []) as $row) {
            ...
            if (!empty($row['break_id'])) {
                ...
                if ($br) {
                    if (!is_null($start)) $br->break_start = $toHis($start);
                    if (!is_null($end))   $br->break_end   = $toHis($end);
                    $br->save();
                }
            } else {
    ...
    public function exportCsv(Request $request, User $user)
    {
        ...
        $toMinutes= function ($time) {
            ...
            try { $c = Carbon::parse($time); } catch (\Throwable $e) { return null; }
